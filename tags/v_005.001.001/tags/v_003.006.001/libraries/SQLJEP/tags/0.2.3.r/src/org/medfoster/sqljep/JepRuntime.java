...
final public class JepRuntime {
    ...
    public JepRuntime(ParserVisitor visitor) {
        ...
        calendar = JepRuntime.threadCalendar.get();
        if (calendar == null) {
            calendar = Calendar.getInstance();
            JepRuntime.threadCalendar.set(calendar);
        }

        dateSymbols = JepRuntime.threadDateFormatSymbols.get();
        if (dateSymbols == null) {
            dateSymbols = new DateFormatSymbols();
            JepRuntime.threadDateFormatSymbols.set(dateSymbols);
        }
    }
}
