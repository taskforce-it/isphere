...
public class DTSDate {
    ...
    private byte[] shiftRight(byte[] bytes, int count) {
        ...
        // printHex("After    ", result);

        return result;
    }

    /**
     * Prints a byte[] as a hex string such as: A6 06 C2 FB ...
     * 
     * @param message - message that is put before the hex string
     * @param bytes - bytes that are printed as a hex string
     */
    private void printHex(String message, byte[] bytes) {
