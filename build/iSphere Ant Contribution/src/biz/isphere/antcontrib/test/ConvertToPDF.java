...
public class ConvertToPDF {
     ...
     * 
     * @param docName An instance of the String class that encapsulates the path
     *        to and name of a valid Word file. Note that there are a few
     *        limitations applying to the format of this String; it must specify
     *        the absolute path to the file and it must not use the single
     *        forward slash to specify the path separator.
     */
    public void openDoc(String docName) {
        Dispatch disp = null;
        Variant var = null;

        // First get a Dispatch object referencing the Documents collection -
        // for
        ...
        // to both open the file and add it to the collection. It would be
        // possible
        // to open a series of files and access each from the Documents
        // collection
     ...
     * the latter and calls the ExportAsFixedFormat method passing the name of
     * the file along with the integer value of 17. This value maps onto one of
     * Word's constants called wdExportFormatPDF and causes the application to
     * convert the file into PDF format. If you wanted to do so, for testing
    ...
    public void publishAsPDF(String filename) {
        ...
        // Object args = new Object{filename, new Integer(17), new
        // Boolean(true)};
    ...
    /**
     * Called to close the active document. Note that this method simply calls
     * the overloaded closeDoc(int) method passing the value 0 which instructs
     * Word to close the document and discard any changes that may have been
     * made since the document was opened or edited.
     */
    public void closeDoc() {
        this.closeDoc(ConvertToPDF.DO_NOT_SAVE_CHANGES);
    }
     ...
     * has made changes to the document that have not been saved. There are
     * three possible value defined by the following manifest constants;
     * DO_NOT_SAVE_CHANGES - Close the document and discard any changes the user
     * may have made. PROMPT_TO_SAVE_CHANGES - Display a prompt to the user
     * asking them how to proceed. SAVE_CHANGES - Save the changes the user has
     * made to the document.
     * 
     * @param saveOption A primitive integer whose value indicates how the close
     *        operation should proceed if the user has made changes to the
     *        active document. Note that no checks are made on the value passed
     *        to this argument.
     */
    ...
    public static void main(String[] args) {
        ...
        String jacobLib = "C:\\workspaces\\rdp_080\\workspace\\iSphere Ant Tasks\\lib\\jacob";
        Properties props = System.getProperties();
        props.setProperty("java.library.path", props.getProperty("java.library.path") + File.pathSeparator + jacobLib);
        
        ConvertToPDF ctpdf = new ConvertToPDF(true);
        ctpdf.openDoc("C:\\workspaces\\rdp_080\\workspace\\iSphere Core Plugin\\doc\\iSphere.doc");
        ctpdf.publishAsPDF("C:\\workspaces\\rdp_080\\workspace\\iSphere Core Plugin\\doc\\iSphere_PDF.pdf");
        ctpdf.closeDoc(ConvertToPDF.PROMPT_TO_SAVE_CHANGES);
        ctpdf.quit();
    }
}