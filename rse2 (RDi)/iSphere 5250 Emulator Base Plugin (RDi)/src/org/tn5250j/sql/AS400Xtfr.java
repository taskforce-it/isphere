...
public class AS400Xtfr {
    ...
    public boolean connect(String host) {
        ...
        try {
            ...
            connection = DriverManager.getConnection("jdbc:as400://" + hostName + ";decimal separator=" + decChar
                + ";extended metadata=true;translate binary=true", user, pass);
            ...
            public void run() {
                try {
                    ...
                    for (int x = 1; x <= numCols; x++) {
                        ...
                        printFTPInfo("Column " + x + ": " + rsmd.getColumnLabel(x) + " " + rsmd.getColumnName(x) + " " + rsmd.getColumnType(x) + " "
                            + rsmd.getColumnTypeName(x) + " " + rsmd.getPrecision(x) + " " + rsmd.getScale(x) + " cn " + rsmd.getCatalogName(x)
                            + " tn " + rsmd.getTableName(x) + " sn " + rsmd.getSchemaName(x));
    ...
    // Exception e = new
    // Exception(ResourceLoader.getText("EXC_TABLE_SPEC_NOT_VALID"));
