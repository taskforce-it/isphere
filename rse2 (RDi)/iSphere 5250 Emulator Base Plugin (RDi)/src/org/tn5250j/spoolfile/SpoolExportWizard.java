...
public class SpoolExportWizard extends GenericTn5250JFrame implements WizardListener {
    ...
    private JPanel pageTwoText() {
        ...
        if (session.getSession().getConfiguration().isPropertyExists("useExternal")) openAfter.setEnabled(true);
    ...
    private boolean pagesValid() {
        ...
        if (pc.isSelected()) {
            if (pcPathInfo.getText().length() == 0) getPCFile();
            if (pcPathInfo.getText().length() == 0) return false;
        }
    ...
    private void cvtToText() {
        ...
        try {
            ...
            // get the text (via a transformed input stream) from the spooled
            // file
            ...
            if (sb.length() > 0) dw.println(sb.toString().toCharArray());
            dw.flush();
            dw.close();
    ...
    public void openOutputFile() {
        ...
        try {
            ...
            if (cvtType.getSelectedIndex() == 0) suffix = ".pdf";
            ...
            if (email.isSelected()) {
                ...
                String tempFile = spooledFile.getText().trim() + '_' + jobName.getText().trim() + '_' + user.getText().trim() + '_'
                    + spooledFileNumber.getText().trim() + '_' + number.getText().trim();
            ...
            if (cvtType.getSelectedIndex() > 0) return;
            ...
            if (document == null) {
                ...
                if (fontSize.getText().length() > 0) fontsize = Float.parseFloat(fontSize.getText().trim());
                ...
                if (author.getText().length() > 0) document.addAuthor(author.getText());
                if (title.getText().length() > 0) document.addTitle(title.getText());
                if (subject.getText().length() > 0) document.addSubject(subject.getText());
