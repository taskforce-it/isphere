...
public class DefaultKeyboardHandler extends KeyboardHandler {
    ...
    protected void displayInfo(KeyEvent e, String s) {
        ...
        String newline = "\n";
        System.out.println(s + newline + "    " + charString + newline + "    " + keyCodeString + newline + "    " + modString + newline + "    "
            + isString + newline);
    ...
    private void processVTKeyPressed(KeyEvent e) {
        ...
        if (lastKeyStroke != null && !lastKeyStroke.equals("null")) {
            ...
            if (lastKeyStroke.startsWith("[mark")) {
                if (lastKeyStroke.equals("[markleft]") || lastKeyStroke.equals("[markright]") || lastKeyStroke.equals("[markup]")
    ...
    private void processVTKeyTyped(KeyEvent e) {
        ...
        if (isLinux) {
            ...
            // if (!((Character.isLetter(kc) || kc == '?') && (e.getModifiers()
            // == 0
            if (!((Character.isLetter(kc) || kc == '\u20AC') && (e.getModifiers() == 0 || e.getModifiers() == KeyEvent.SHIFT_MASK))) {
        ...
        screen.sendKeys(s);
        if (recording) recordBuffer.append(s);
        keyProcessed = true;
        e.consume();
    }
    ...
    private void processVTKeyReleased(KeyEvent e) {
        ...
        if (Character.isISOControl(e.getKeyChar()) || keyProcessed || e.isConsumed()) return;
