...
public class FileFieldDef {
    ...
    public String parseData(byte[] cByte) {
        ...
        case 'P': // Packed decimal format
            ...
            for (int f = startOffset - 1; f < end - 1; f++) {
                ...
                sb.append(byteD); // assign the digit portion as the second
                                  // digit
            }
    ...
    public void setFieldData(String fd) {
        if (sbdata == null) sbdata = new StringBuffer(length);
        sbdata.setLength(0);
        sbdata.append(fd);
    }
    ...
    public String toString() {
        return fieldName + " " + startOffset + " " + length + " " + getBufferOutLength() + " " + numDigits + " " + decPos + " " + type + " "
            + txtDesc;
    }
