...
/**
 * This is taken from Sun's demo ExampleFileFiler.java A convenience
 * implementation of FileFilter that filters out all files except for those type
 * extensions that it knows about. Extensions are of the type ".foo", which is
 * typically found on Windows and Unix boxes, but not on Macinthosh. Case is
 * ignored. Example - create a new filter that filerts out all files but gif and
 * jpg image files: JFileChooser chooser = new JFileChooser(); XTFRFileFilter
 * filter = new XTFRFileFilter( new String{"gif", "jpg"}, "JPEG & GIF Images")
...
public class XTFRFileFilter extends FileFilter {
     ...
     * XTFRFileFilter("jpg", "JPEG Image Images"); Note that the "." before the
     ...
     * XTFRFileFilter(String {"gif", "jpg"}); Note that the "." before the
     ...
     * those that end in ".jpg" and ".tif": XTFRFileFilter filter = new
     * XTFRFileFilter(); filter.addExtension("jpg"); filter.addExtension("tif");
     * Note that the "." before the extension is not needed and will be ignored.
     */
    ...
    /**
     * Set the extension to be used for this type if one is not provided This
     * will append the first key of the filter contained in the list
     */
