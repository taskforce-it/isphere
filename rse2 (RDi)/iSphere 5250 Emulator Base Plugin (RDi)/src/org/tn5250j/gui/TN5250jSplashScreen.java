...
/**
 * Uses an Icon or the location of an image to create an application's
 * introductory screen.
 */
public class TN5250jSplashScreen extends Canvas {
    ...
    /**
     * Creates a splash screen given the location of the image. The image
     * location is the package path of the image and must be in the classpath.
     * For example, if an image was located in /test/examples/image.gif, and the
     * classpath specified contains /test, the constructor should be passed
     * "/examples/image.gif".
     */
    ...
    protected void initialize(ImageIcon iimage) {
        ...
        Dimension s = new Dimension(image.getWidth(this) + 2, image.getHeight(this) + 2);
        setSize(s);
    ...
    public synchronized void paint(Graphics g) {
        ...
        offScreenBufferGraphics.fill3DRect(inset - 1, image.getHeight(this) - (height + 2), image.getWidth(this) - (inset * 2), height + 1, false);

        // draw progress
        offScreenBufferGraphics.setColor(new Color(204, 204, 255));
        offScreenBufferGraphics.fillRect(inset, image.getHeight(this) - (height + 1), ((image.getWidth(this) - (inset * 2)) / steps) * progress,
            height);
    ...
    /**
     * This method will show or hide the splash screen. Once the splash screen
     * is hidden, the splash screen window will be disposed. This means the
     * splash screen cannot become visible again.
     */
    @Override
