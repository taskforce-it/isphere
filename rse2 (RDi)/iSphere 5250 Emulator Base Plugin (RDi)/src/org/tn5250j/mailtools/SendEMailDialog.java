...
public class SendEMailDialog extends GenericTn5250JFrame implements Runnable {
    ...
    public SendEMailDialog(Frame parent, SessionPanel session, boolean sendScreen) {
        ...
        if (!isEMailAvailable()) {
            ...
            JOptionPane.showMessageDialog(parent, LangTool.getString("messages.noEmailAPI"), "Error", JOptionPane.ERROR_MESSAGE, null);
            ...
            while (result == 0 || result == 2) {
                ...
                case 0: // Send it
                    ...
                    if (bodyText.getText().length() > 0) sendEMail.setMessage(bodyText.getText());

                    if (attachmentName.getText().length() > 0) if (!normal.isSelected())
                    ...
                    if (text.isSelected()) {
                        ...
                        for (int x = 0; x < l; x++, col++) {
                            ...
                            if (screenTxt[x] >= ' ' && ((screenExtendedAttr[x] & TN5250jConstants.EXTENDED_5250_NON_DSP) == 0)) {
                                ...
                                if ((screenExtendedAttr[x] & TN5250jConstants.EXTENDED_5250_UNDERLINE) != 0 && screenAttrPlace[x] != 1) {
                                    sb.append('_');
                                } else {
        ...
        if (!isEMailAvailable()) {
            ...
            while (result == 0 || result == 2) {
                ...
                case 0: // Send it
                    ...
                    if (attachmentName.getText().length() > 0) sendEMail.setAttachmentName(attachmentName.getText());

                    if (fileName != null && fileName.length() > 0) sendEMail.setFileName(fileName);
    ...
    public void run() {
        ...
        try {
            if (sendEMail.send()) {
                ...
                JOptionPane.showMessageDialog(null, LangTool.getString("em.confirmationMessage") + " " + (String)toAddress.getSelectedItem(),
                    LangTool.getString("em.titleConfirmation"), JOptionPane.INFORMATION_MESSAGE);

                if (session != null) {
                    config.setProperty("emailTo", getToTokens(config.getStringProperty("emailTo"), toAddress));
                    config.saveSessionProps();
                    setToCombo(config.getStringProperty("emailTo"), toAddress);
    ...
    /**
     * Set the combo box items to the string token from to. The separator is a
     * '|' character.
     * 
     * @param to
     * @param boxen
    ...
    /**
     * Creates string of tokens from the combobox items. The separator is a '|'
     * character. It does not save duplicate items.
     * 
     * @param to
     * @param boxen
     * @return
    ...
    private String getToTokens(String to, JComboBox boxen) {
        ...
        for (int x = 0; x < c; x++) {
            if (!selected.equals(boxen.getItemAt(x))) sb.append((String)boxen.getItemAt(x) + '|');
        }
        return sb.toString();
    }
