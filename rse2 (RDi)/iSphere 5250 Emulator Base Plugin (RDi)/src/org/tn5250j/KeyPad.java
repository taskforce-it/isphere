...
public class KeyPad extends JPanel {
    ...
    /*
     * These rectangles/insets are allocated once for all calls. Re-using
     * rectangles rather than allocating them in each paint call substantially
     * reduced the time it took paint to run.
     */
    ...
    */
    protected void paintComponent(Graphics g) {
        ...
        while (SwingUtilities.layoutCompoundLabel(fm, buttons[bSize - 1].getText(), null, buttons[0].getVerticalAlignment(),
            buttons[0].getHorizontalAlignment(), buttons[0].getVerticalTextPosition(), buttons[0].getHorizontalTextPosition(), viewRect, textRect,
            iconRect, 0).endsWith(endsWithSuffix)
            && fs > (minSize - 1)) {
            k = k.deriveFont(--fs);
