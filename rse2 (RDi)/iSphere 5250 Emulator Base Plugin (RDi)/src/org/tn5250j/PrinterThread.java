...
public class PrinterThread extends Thread implements Printable {
    ...
    public PrinterThread(Screen5250 scr, Font font, int cols, int rows, Color colorBg, boolean toDefaultPrinter, SessionPanel ses) {
    ...
    public void run() {
        ...
        // new int[] { 1, 1 }
        // };
        // JobAttributes jobAttributes = new JobAttributes(1,
        // JobAttributes.DefaultSelectionType.ALL,
        // JobAttributes.DestinationType.PRINTER, JobAttributes.DialogType.NONE,
        // "file", 1, 1,
        // JobAttributes.MultipleDocumentHandlingType.SEPARATE_DOCUMENTS_COLLATED_COPIES,
        // range, "HP LaserJet", JobAttributes.SidesType.ONE_SIDED);
        ...
        if (numCols != 132) {
            if (config.getStringProperty("print.portWidth").length() != 0 && config.getStringProperty("print.portHeight").length() != 0
                && config.getStringProperty("print.portImageWidth").length() != 0 && config.getStringProperty("print.portImageHeight").length() != 0
                && config.getStringProperty("print.portImage.X").length() != 0 && config.getStringProperty("print.portImage.Y").length() != 0) {

                Paper paper = pf.getPaper();

                paper.setSize(Double.parseDouble(config.getStringProperty("print.portWidth")),
                    Double.parseDouble(config.getStringProperty("print.portHeight")));

                paper.setImageableArea(Double.parseDouble(config.getStringProperty("print.portImage.X")),
            ...
            if (config.getStringProperty("print.landWidth").length() != 0 && config.getStringProperty("print.landHeight").length() != 0
                && config.getStringProperty("print.landImageWidth").length() != 0 && config.getStringProperty("print.landImageHeight").length() != 0
                && config.getStringProperty("print.landImage.X").length() != 0 && config.getStringProperty("print.landImage.Y").length() != 0) {

                Paper paper = pf.getPaper();

                paper.setSize(Double.parseDouble(config.getStringProperty("print.landWidth")),
                    Double.parseDouble(config.getStringProperty("print.landHeight")));

                paper.setImageableArea(Double.parseDouble(config.getStringProperty("print.landImage.X")),
        ...
        if (config.getStringProperty("print.font").length() > 0) {
            ...
            font = new Font(config.getStringProperty("print.font"), Font.PLAIN, 8);
        }
     ...
     * Method: print
     * <p>
     * This routine is responsible for rendering a page using the provided
     * parameters. The result will be a screen print of the current screen to
    ...
    public int print(Graphics g, PageFormat pageFormat, int page) {
        ...
        if (page == 0) {
            ...
            int w = (int)pageFormat.getImageableWidth() / numCols; // proposed
                                                                   // width
            int h = (int)pageFormat.getImageableHeight() / numRows; // proposed
                                                                    // height
            ...
            for (; j < 50; j++) {
                ...
                // derive the font and obtain the relevent information to
                // compute
                ...
                if ((w < (int)k.getStringBounds("W", f).getWidth())
                    || h < (int)(k.getStringBounds("y", f).getHeight() + l.getDescent() + l.getLeading())

                ) break;
            }

            // since we were looking for an overrun of the width or height we
            // need
            ...
            int h1 = (int)(k.getStringBounds("y", f).getHeight() + l.getDescent() + l.getLeading());
            int x;
            int y;
            ...
            for (int m = 0; m < numRows; m++)
                for (int i = 0; i < numCols; i++) {
                    ...
                    if ((screenExtendedAttr[getPos(m, i)] & TN5250jConstants.EXTENDED_5250_UNDERLINE) != 0 && screenAttrPlace[getPos(m, i)] != 1)
