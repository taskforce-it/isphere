...
public class SessionPopup {
    ...
    public SessionPopup(SessionPanel ses, MouseEvent me) {
        ...
        if (!sessiongui.rubberband.isAreaSelected() && screen.isInField(pos, false)) {
            ...
            if (screen.getOIA().isMessageWait() && OptionAccessFactory.getInstance().isValidOption(MNEMONIC_DISP_MESSAGES)) {
            ...
            if (sessiongui.isMacroRunning()) {
                ...
                if (Macronizer.isMacrosExist()) {
                    // this will add a sorted list of the macros to the macro
                    // menu
                    addMacros(macMenu);
                }
                popup.add(macMenu);
        ...
        /*
         * if
         * (OptionAccessFactory.getInstance().isValidOption(MNEMONIC_OPEN_NEW))
         * { action = new
         * AbstractAction(LangTool.getString("popup.connections")) { private
    ...
    private void createShortCutItems(JMenu menu) {
        ...
        for (int x = 0; x < allKeys.length; x++) {
            ...
            // ISPHERE - NEW - START
            String _key = (String)map.get(allKeys[x]);
    ...
    private void sumArea(boolean which) {
        ...
        JOptionPane.showMessageDialog(null, df.format(sum), LangTool.getString("popup.calc"), JOptionPane.INFORMATION_MESSAGE);
    ...
    private void doMeSpool() {
        ...
        try {
            org.tn5250j.spoolfile.SpoolExporter spooler = new org.tn5250j.spoolfile.SpoolExporter(vt, sessiongui);
            spooler.setVisible(true);
        } catch (NoClassDefFoundError ncdfe) {
            JOptionPane.showMessageDialog(sessiongui, LangTool.getString("messages.noAS400Toolbox"), "Error", JOptionPane.ERROR_MESSAGE, null);
        }
