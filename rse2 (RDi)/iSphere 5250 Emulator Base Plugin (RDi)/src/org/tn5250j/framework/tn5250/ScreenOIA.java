...
public class ScreenOIA {
    ...
    public void setKeysBuffered(boolean kb) {
        ...
        keysBuffered = kb;
        if (keysBuffered != oldKB) fireOIAChanged(ScreenOIAListener.OIA_CHANGED_KEYS_BUFFERED);
    }
    ...
    protected void setKeyBoardLocked(boolean lockIt) {
        ...
        if (locked != oldLocked) fireOIAChanged(ScreenOIAListener.OIA_CHANGED_KEYBOARD_LOCKED);
    }
    ...
    private void fireOIAChanged(int change) {
        ...
        if (listeners != null) {
            ...
            for (int i = 0; i < size; i++) {
                ScreenOIAListener target = listeners.elementAt(i);
