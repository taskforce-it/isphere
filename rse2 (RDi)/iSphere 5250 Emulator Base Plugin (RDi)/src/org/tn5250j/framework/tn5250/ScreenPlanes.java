...
public class ScreenPlanes {
    ...
    protected void setErrorLine(int line) {
        ...
        // parameters to the host so check setsize for setting error line as
        // well.
        //
    ...
    protected final void setChar(int pos, char c) {
        ...
        screen[pos] = c;
        if (screenIsAttr[pos] == 1) setScreenCharAndAttr(pos, c, 32, false);
    ...
    private void disperseAttribute(int pos, int attr) {
        ...
        case 63: // nondisplay
            ...
            break;
        default:
            c = (COLOR_BG_BLACK << 8 & 0xff00) | (COLOR_FG_YELLOW & 0xff);
            break;
    ...
    protected void initalizePlanes() {
        ...
        char c = (COLOR_BG_BLACK << 8 & 0xff00) | (COLOR_FG_GREEN & 0xff);
    ...
    /**
     * Converts a linear presentation space position to its corresponding
     * column.
     * 
     * @param pos The position to be converted
    ...
    /**
     * Converts a row and column coordinate to its corresponding linear
     * position.
     * 
    ...
    protected boolean checkHotSpots() {
        ...
        for (int x = 0; x < lenScreen; x++) {
            ...
            if (x > 0 && screen[x] == 'F') {
                if (screen[x + 1] >= '0' && screen[x + 1] <= '9' && screen[x - 1] <= ' ' && (screenExtended[x] & EXTENDED_5250_NON_DSP) == 0) {

                    if (screen[x + 2] >= '0' && screen[x + 2] <= '9' && (screen[x + 3] == '=' || screen[x + 3] == '-' || screen[x + 3] == '/'))
                        hs = true;
                    else if (screen[x + 2] == '=' || screen[x + 3] == '-' || screen[x + 3] == '/') hs = true;
                    ...
                    if (hs) {
                        ...
                        while (ns < 2 && ++x / numCols == row) {
                            if (screen[x] <= ' ')
                                ...
                                ns = 0;
                            if (ns < 2) screenGUI[x] = BUTTON_MIDDLE;

                        }

                        // now lets go back and take out gui's that do not
                        // belong
                        while (screen[--x] <= ' ') {
                            screenGUI[x] = NO_GUI;
                        }
            ...
            if (!hs && x > 0 && x < lenScreen - 2 && screen[x] == '.' && screenGUI[x] == NO_GUI && (screenExtended[x] & EXTENDED_5250_UNDERLINE) == 0
                ...
                if ((os = isOption(screen, x, lenScreen, 2, 3, '.')) > 0) {
                    ...
                    while (++stop / numCols == row && (screen[stop] >= ' ' || screen[stop] == 0x0)) {
            ...
            if (!hs && x > 0 && x < lenScreen - 2 && screen[x] == '=' && screenGUI[x] == NO_GUI && (screenExtended[x] & EXTENDED_5250_UNDERLINE) == 0
                ...
                if ((os = isOption(screen, x, lenScreen, 2, 2, '=')) > 0) {
                    ...
                    while (++stop / numCols == row && screen[stop] >= ' ') {
            ...
            if (!hs && x > 2 && x < lenScreen - hsMore.length() && screen[x] == hsMore.charAt(0) && screen[x - 1] <= ' ' && screen[x - 2] <= ' '
                && screenGUI[x] == NO_GUI && (screenExtended[x] & EXTENDED_5250_NON_DSP) == 0) {
            ...
            if (!hs && x > 2 && x < lenScreen - hsBottom.length() && screen[x] == hsBottom.charAt(0) && screen[x - 1] <= ' ' && screen[x - 2] <= ' '
                && screenGUI[x] == NO_GUI && (screenExtended[x] & EXTENDED_5250_NON_DSP) == 0) {
            ...
            if (!hs && x > 0 && x < lenScreen - 7 && Character.toLowerCase(screen[x]) == 'h' && screen[x - 1] <= ' ' && screenGUI[x] == NO_GUI
                && (screenExtended[x] & EXTENDED_5250_NON_DSP) == 0) {

                if (Character.toLowerCase(screen[x + 1]) == 't' && Character.toLowerCase(screen[x + 2]) == 't'
                    && Character.toLowerCase(screen[x + 3]) == 'p' && screen[x + 4] == ':' && screen[x + 5] == '/' && screen[x + 6] == '/') {
                ...
                else if (Character.toLowerCase(screen[x + 1]) == 't' && Character.toLowerCase(screen[x + 2]) == 't'
                    && Character.toLowerCase(screen[x + 3]) == 'p' && Character.toLowerCase(screen[x + 4]) == 's' && screen[x + 5] == ':'
                    && screen[x + 6] == '/' && screen[x + 7] == '/') {
            ...
            else if (!hs && x > 0 && x < lenScreen - 7 && Character.toLowerCase(screen[x]) == 'm' && screen[x - 1] <= ' ' && screenGUI[x] == NO_GUI
                && (screenExtended[x] & EXTENDED_5250_NON_DSP) == 0) {

                if (Character.toLowerCase(screen[x + 1]) == 'a' && Character.toLowerCase(screen[x + 2]) == 'i'
                    && Character.toLowerCase(screen[x + 3]) == 'l' && Character.toLowerCase(screen[x + 4]) == 't'
                    && Character.toLowerCase(screen[x + 5]) == 'o' && screen[x + 6] == ':') {
            ...
            else if (!hs && x > 0 && x < lenScreen - 7 && screen[x - 1] <= ' ' && screenGUI[x] == NO_GUI
                && (screenExtended[x] & EXTENDED_5250_NON_DSP) == 0) {
    ...
    private int isOption(char[] screen, int x, int lenScreen, int numPref, int numSuff, char suff) {
        ...
        while (--sp >= 0 && screen[sp] <= ' ') {
            ...
            if (x - sp > numPref || screen[sp] == suff || screen[sp] == '.' || screen[sp] == '*') {
                hs = false;
                break;
            }
        }

        // now lets check for how long the option is it has to be numPref or
        // less
        os = sp;
        while (hs && --os > 0 && screen[os] > ' ') {

            if (sp - os >= numPref || screen[os] == suff || screen[os] == '.' || screen[os] == '*') {
                hs = false;
                break;
            }
        }
        ...
        if (Character.isDigit(screen[sp + 1])) hs = false;
        // now lets make sure there are no more than numSuff spaces after option
        while (hs && (++sp < lenScreen && screen[sp] <= ' ' || screen[sp] == suff)) {
            if (sp - x >= numSuff || screen[sp] == suff || screen[sp] == '.' || screen[sp] == '*') {
                hs = false;
                break;
            }
        }
        if (hs && !Character.isLetterOrDigit(screen[sp])) hs = false;
        if (hs) {
            return os;
        }
        return -1;
