...
public class DataStreamProducer implements Runnable {
    ...
    public final void run() {
        ...
        while (!done) {
            try {
                ...
                // WVL - LDC : 17/05/2004 : Device name negotiations send TIMING
                // MARK
    ...
    private void loadStream(byte streamBuffer[], int offset) {
        ...
        if (partialLen > bufferLen) {
            ...
            try {
                dsq.put(buf);
                if (streamBuffer.length > buf.length + offset + MINIMAL_PARTIAL_STREAM_LEN) loadStream(streamBuffer, offset + buf_len);
    ...
    public final byte[] readIncoming() throws IOException {
        ...
        while (!done) {
            ...
            //
            // my5250
            // 0000: 00 50 DA 44 C8 45 42 00 00 00 00 24 08 00 45 00
            // .P.D.EB....$..E.
            // 0010: 04 2A BC F9 00 00 40 06 D0 27 C1 A8 33 04 C1 A8
            // .*....@..'..3...
            // 0020: 33 58 00 17 04 18 6F A2 83 CB 00 1E D1 BA 50 18
            // 3X....o.......P.
