...
public final class tnvt implements Runnable {
     ...
     * 
     * @param sr system request option (allowed to be null, but than nothing
     *        happens)
     */
     ...
     * encountered. When found the Session#execCommand(String, int) is called
     * with the parsed string. The position immediately following the
     * encountered white space, separating the command from the rest of the
     * screen, is passed as starting index. Note that the character at the
     * starting position can potentially be a white space itself. The starting
     * position in <code>execCommand</code> provided to make the scanning
     * sequence more flexible. We'd like for example to embed also a
     * <code>+</code> or <code>-</code> sign to indicate whether the tnvt should
     * trigger a repaint or not. This would allow the flashing of command
     * sequences without them becoming visible.
     * <ul>
        ...
        try {
            ...
            for (int i = 0; i < len; i++) { // save the screen data
                ...
                if (planes.isAttributePlace(i)) {
                    ...
                    // LDC: Check to see if it is an displayable character. If
                    // not,
                    // do not convert the character.
                    // The characters on screen are in unicode
                    // sa[sac++] =
    ...
    public final void restoreScreen() throws IOException {
        ...
        try {
            ...
            if (sfParser != null && sfParser.isGuisExists()) sfParser.clearGuiStructs();
            ...
            for (int y = 0; y < len; y++) {
                ...
                if (b == 0x04) {
                    ...
                    log.info(" gui restored at " + y + " - " + screen52.getRow(y) + "," + screen52.getCol(y));
                    ...
                    if (isAttribute(b)) {
                        ...
                        // LDC - 12/02/2003 - Check to see if it is an
                        // displayable
                        // character. If not,
                        // do not convert the character.
                        // The characters on screen are in unicode
                        char ch = (char)b;
                        if (isDataEBCDIC(b)) ch = codePage.ebcdic2uni(b);
            ...
            screen52.setPendingInsert(true, screen52.getRow(pos + cols), screen52.getCol(pos + cols));
    ...
    /**
     * This routine handles sending negative responses back to the host. You can
     * find a description of the types of responses to be sent back by looking
     * at section 12.4 of the 5250 Functions Reference manual
     * 
     * @param cat
     * @param modifier
     * @param uByte1
            ...
            while (bk.hasNext() && !done) {
                ...
                case 02: // RA - Repeat to address
                    ...
                    if (toRow >= row) {
                        ...
                        if (row == 1 && col == 2 && toRow == screen52.getRows() && toCol == screen52.getColumns())
                ...
                case 03: // EA - Erase to address
                    ...
                    // a little intelligence here I hope
                    if (EArow == 1 && EAcol == 2 && toEARow == screen52.getRows() && toEACol == screen52.getColumns())

                        screen52.clearScreen();
                    else {
                        int times = ((toEARow * screen52.getColumns()) + toEACol) - ((EArow * screen52.getColumns()) + EAcol);
                        while (times-- >= 0) {
                ...
                case 17: // SBA - set buffer address order (row column)
                    ...
                    if (saRow >= 0 && saRow <= screen52.getRows() && saCol >= 0 && saCol <= screen52.getColumns()) {
                        ...
                        sendNegResponse(NR_REQUEST_ERROR, 0x05, 0x01, 0x22,
                            "invalid row/col order" + " saRow = " + saRow + " saRows = " + screen52.getRows() + " saCol = " + saCol);
                ...
                case 20: // MC - Move Cursor
                    ...
                    if (imcX >= 0 && imcX <= saRows && imcY >= 0 && imcY <= saCols) {
                    ...
                    if ((ffw0 & 0x40) == 0x40) {
                        ...
                        if (!isAttribute(fcw1)) {
                            ...
                            while (!isAttribute(attr)) {
                                log.info(Integer.toHexString(fcw1) + " " + Integer.toHexString(fcw2) + " ");
                                log.info(Integer.toHexString(attr) + " " + Integer.toHexString(bk.getNextByte() & 0xff));
                                // bk.getNextByte();
    ...
    private void processCC1(byte byte1) {
        ...
        if ((byte1 & 0x20) == 0x20 && (byte1 & 0x08) == 0x00) {
            ...
            log.debug(" WTD position move to home" + screen52.homePos + " row " + screen52.getRow(screen52.homePos) + " col "
    ...
    /**
     * Method sendQueryResponse The query command is used to obtain information
     * about the capabilities of the 5250 display. The Query command must follow
     * an Escape (0x04) and Write Structured Field command (0xF3). This section
     * is modeled after the rfc1205 - 5250 Telnet Interface section 5.3
     * 
     * @throws IOException
     */
