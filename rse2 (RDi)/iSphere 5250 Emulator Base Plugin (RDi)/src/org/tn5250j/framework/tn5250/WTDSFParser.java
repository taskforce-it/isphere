...
/**
 * Write To Display Structured Field: This module will parse the structured
 * field information for enhanced emulation mode.
 */
public class WTDSFParser {
        ...
        while (!done) {
            ...
            case 0xD9: // Class Type 0xD9 - Create Window
                ...
                case 0x51: // Create Window
                    ...
                    if ((segment[pos++] & 0x80) == 0x80) cr = true; // restrict
                                                                    // cursor
                    ...
                    if (length == 0) {
                        ...
                        // System.out.println(" Depth = " + rows + " Width = " +
                        // cols);
                    ...
                    while (length > 0) {
                        ...
                        case 0x01: // Border presentation
                            boolean gui = false;
                            if ((segment[pos++] & 0x80) == 0x80) gui = true;
                            mAttr = segment[pos++];
                            cAttr = segment[pos++];
                            ...
                            // System.out.println("Create Window");
                            // System.out.println("   restrict cursor " + cr);
                            // System.out.println("   Depth = " + rows +
                            // " Width = " + cols);
                            // System.out.println("   type = " + type +
                            // " gui = " + gui);
                            // System.out.println("   mono attr = " + mAttr +
                            // " color attr = " + cAttr);
                            // System.out.println("   ul = " + ul + " upper = "
                            // + upper +
                            // " ur = " + ur +
                            // " left = " + left +
                            // " right = " + right +
                            ...
                            createWindow(rows, cols, type, gui, mAttr, cAttr, ul, upper, ur, left, right, ll, bottom, lr);
                            windowDefined = true;
                            break;
                        //
                        ...
                        // .?.?..?...?..?.
                        // +0010 00000018 00000011 06131500 37D95180
                        // ........?.?..R??
                        // +0020 00000A24 0D018023 23404040 40404040 ..??..???
                        // +0030 40211000 000000D7 C2C1D9C4 C5D4D67A
                        // \uFFFD.....PBARDEMO:
                        // +0040 40D79996 879985A2 A2408281 99408485 Progress
                        // bar de
                        // +0050 94961108 1520D5A4 94828599 40968640
                        // mo.???Number of
                        // +0060 8595A399 8985A24B 4B4B4B4B 4B7A2011
                        // entries......:?.
                        // +0070 082E2040 404040F5 F0F06BF0 F0F02011 ?.?
                        // 500,000?.
                        // +0080 091520C3 A4999985 95A34085 95A399A8
                        // \uFFFD??Current entry
                        // +0090 4095A494 8285994B 4B4B7A20 11092E20
                        // number...:?.\uFFFD.?
                        // +00A0 40404040 4040F56B F0F0F020 110A1520 5,000?.???
                        // +00B0 D9859481 89958995 87408595 A3998985 Remaining
                        // entrie
                        // +00C0 A24B4B4B 4B4B4B7A 20110A2E 20404040
                        // s......:?.?.?
                        // +00D0 40F4F9F5 6BF0F0F0 20110C15 20E2A381
                        // 495,000?..??Sta
                        // +00E0 99A340A3 8994854B 4B4B4B4B 4B4B4B4B rt
                        // time.........
                        // +00F0 4B4B4B4B 7A20110C 2F2040F7 7AF5F37A ....:?...?
                        // 7:53:
                        ...
                        case 0x10: // Window title/footer
                            ...
                            log.debug(" orientation " + Integer.toBinaryString(orientation) + " mAttr " + mAttr + " cAttr " + cAttr
                ...
                case 0x53: // Scroll Bar
                    ...
                    // position 7,8
                    int totalRowScrollable = ((segment[pos++] & 0xff) << 8 | (segment[pos++] & 0xff));

                    // position 9,10
                    int totalColScrollable = ((segment[pos++] & 0xff) << 8 | (segment[pos++] & 0xff));

                    // position 11,12
                    int sliderRowPos = ((segment[pos++] & 0xff) << 8 | (segment[pos++] & 0xff));

                    // position 13,14
                    int sliderColPos = ((segment[pos++] & 0xff) << 8 | (segment[pos++] & 0xff));
                ...
                case 0x60: // Erase/Draw Grid Lines - not supported
                           ...
                           // just in case.
                           // System.out.println("erase/draw grid lines " +
                           // length);
                    len = 6;
                    d = 0;
                    length -= 9;
                    ...
                    break;
                default:
                    vt.sendNegResponse(NR_REQUEST_ERROR, 0x03, 0x01, 0x01, "invalid wtd structured field sub command " + (pos - 1));
                    // + bk.getByteOffset(-1));
                    error = true;
                    break;
                }
            ...
            default:
                vt.sendNegResponse(NR_REQUEST_ERROR, 0x03, 0x01, 0x01, "invalid wtd structured field command " + (pos - 1));
                // + bk.getByteOffset(-1));
                error = true;
                break;
            }
    ...
    /* *** NEVER USED LOCALLY ************************************************** */
    // private void clearWindowBody(ScreenPlanes planes, int startPos, int
    // depth, int width) {
    //
    ...
    private void defineSelectionField(int majLen) {
        ...
        // .@...(..........
        // 0050: 14 19 15 00 48 D9 50 00 60 00 11 01 84 84 00 00
        // ....H.P.`.......
        ...
        // ...@@.........@@
        // 0090: 0B 10 08 00 E0 00 E3 88 99 85 85 04 52 00 00 FF
        // ............R...
        // 00A0: EF .
        try {
            ...
            int typeSelection = segment[pos++]; // Type of selection Field -
                                                // byte 8
            ...
            int minLen = 0;
            majLen -= 21;
            log.debug(" row: " + screen52.getCurrentRow() + " col: " + screen52.getCurrentCol() + " type " + typeSelection + " gui " + guiDevice
                + " withMnemonic " + Integer.toHexString(withMnemonic & 0xf0) + " noMnemonic " + Integer.toHexString(noMnemonic & 0xf0)
                + " noMnemonic " + Integer.toBinaryString(noMnemonic) + " noMnemonicType " + Integer.toBinaryString((noMnemonic & 0xf0))
                + " noMnemonicSel " + Integer.toBinaryString((noMnemonic & 0x0f)) + " maxcols " + maxColChoice + " cols " + cols + " rows " + rows);
            int rowCtr = 0;
            int colCtr = 0;
            ...
            // client access
            // 0000 00 04 ac 9e b9 35 00 01 02 32 bb 4e 08 00 45 00
            // .....5...2.N..E.
            // 0010 00 3c 4f 8e 40 00 80 06 00 00 c1 a8 33 58 c1 a8
            // .<O.@.......3X..
            // 0020 33 01 09 e4 00 17 5b bf b7 a4 c3 41 43 d1 50 18
            // 3.....[....AC.P.
            ...
            do {
                ...
                case 0x10: // Choice Text minor structure
                    ...
                    // System.out.println(Integer.toBinaryString((flagCT1 &
                    // 0xf0)));
                    ...
                    if ((flagCT1 & 0x80) != 0x80) {
                        ...
                        screen52.getScreenFields().getCurrentField().setSelectionFieldInfo(17, fld, chcPos);
