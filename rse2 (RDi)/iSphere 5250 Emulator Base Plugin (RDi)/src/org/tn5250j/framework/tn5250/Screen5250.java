...
public class Screen5250 {
    ...
    public final void pasteText(String content, boolean special) {
        ...
        for (int x = nextChar; x < nChars; x++) {
            ...
            if ((c == '\n') || (c == '\r')) {
                ...
                // If we go paste the end of the screen then let's start over
                // from
                // the beginning of the screen space.
                if (cpos > length) cpos = 0;
            } else {
                ...
                // characters to paste.
                if (special && (!Character.isLetter(c) && !Character.isDigit(c))) setIt = false;

                // we will only push a character to the screen space if we are
                // in
                // a field
        ...
        if (pd.length() > 0) log.info("pasted >" + pd + "<");
    ...
    public final Vector<Double> sumThem(boolean which, Rect area) {
        ...
        while (workR.height-- > 0) {
            ...
            while (t-- > 0) {
                ...
                // TODO: update me here to implement the nonDisplay check as
                // well
                if (((c >= '0' && c <= '9') || c == '.' || c == ',' || c == '-')) {
                    sb.append(c);
                }
            ...
            if (sb.length() > 0) {
                ...
                try {
                    ...
                    log.warn(pe.getMessage() + " at " + pe.getErrorOffset());
                }
            }
            sb.setLength(0);
            m++;
    ...
    /**
     * This will move the screen cursor based on the mouse event. I do not think
     * the checks here for the gui characters should be here but will leave them
     * here for now until we work out the interaction. This should be up to the
     * gui frontend in my opinion.
     * 
     * @param pos
     */
        ...
        if (!oia.isKeyBoardLocked()) {
            ...
            if (g >= BUTTON_LEFT && g <= BUTTON_LAST) {
                ...
                case BUTTON_LEFT:
                    ...
                    if (planes.getChar(pos + 1) != '=' && planes.getChar(pos + 1) != '.' && planes.getChar(pos + 1) != '/') {
                        ...
                        log.debug(" Hotspot clicked!!! we will send character " + planes.getChar(pos));
                        aid.append(planes.getChar(pos));
                    }
                    break;
                ...
                if (aidFlag) {
                    ...
                    default:
                        ...
                        if (aidKey >= 1 && aidKey <= 12) sessionVT.sendAidKey(0x30 + aidKey);
                        if (aidKey >= 13 && aidKey <= 24) sessionVT.sendAidKey(0xB0 + (aidKey - 12));
                    }
                } else {
                ...
                // return back to the calling routine that the cursor was not
                // moved
    ...
    private int getMnemonicValue(String mnem) {
        ...
        for (int x = 0; x < mnemonicData.length; x++) {
            ...
            if (mnemonicData[x].equals(mnem)) return mnemonicValue[x];
        }
        return 0;
     ...
     * [pf1] pf1 etc... These will be processed as if you had pressed these keys
     * from the keyboard. All the valid special key values are contained in the
     ...
     * @see #sendAid Added synchronized to fix a StringOutOfBounds error - Luc
     *      Gorren LDC
     */
    ...
    protected boolean simulateMnemonic(int mnem) {
        ...
        case BACK_SPACE:
            if (screenFields.getCurrentField() != null && screenFields.withinCurrentField(lastPos) && !screenFields.isCurrentFieldBypassField()) {
        ...
        case BACK_TAB:
            ...
            if (screenFields.isCurrentFieldContinued()) {
                do {
                    gotoFieldPrev();
                } while (screenFields.isCurrentFieldContinuedMiddle() || screenFields.isCurrentFieldContinuedLast());
            }
            isInField(lastPos);
            simulated = true;
            break;
        ...
        case TAB:
            ...
            if (screenFields.getCurrentField() != null && !screenFields.isCurrentFieldContinued()) {
                ...
                do {
                    ...
                    && (screenFields.isCurrentFieldContinuedMiddle() || screenFields.isCurrentFieldContinuedLast()));
            }
            ...
            if (screenFields.getCurrentField() != null && screenFields.withinCurrentField(lastPos) && !screenFields.isCurrentFieldBypassField()) {
                ...
                if (fieldExit()) {
                    ...
                    if (!screenFields.isCurrentFieldContinued()) {
                        ...
                        do {
                            gotoFieldNext();
                            if (screenFields.isCurrentFieldContinued()) fieldExit();
                        } while (screenFields.isCurrentFieldContinuedMiddle() || screenFields.isCurrentFieldContinuedLast());
                    }
                }
                updateDirty();
            ...
            if (screenFields.getCurrentField() != null && screenFields.withinCurrentField(lastPos) && !screenFields.isCurrentFieldBypassField()) {
                ...
                if (s == 3 || s == 5 || s == 7) {
                    ...
                    if (fieldExit()) {
                        ...
                        simulated = true;
                        updateDirty();
                        if (screenFields.isCurrentFieldAutoEnter()) sendAid(AID_ENTER);
        ...
        case NEW_LINE:
            if (screenFields.getSize() > 0) {
                ...
                if (startRow == getRows()) startRow = 0;

                setCursor(++startRow, 1);

                if (!isInField() && screenFields.getCurrentField() != null && !screenFields.isCurrentFieldBypassField()) {
        ...
        case FAST_CURSOR_DOWN:
            ...
            if (rowNow > getRows() - 1) rowNow = rowNow - getRows();
            this.goto_XY(getPos(rowNow, getCurrentCol() - 1));
            simulated = true;
            break;
        case FAST_CURSOR_UP:
            rowNow = (getCurrentRow() - 1) - 3;
            if (rowNow < 0) rowNow = (getRows()) + rowNow;
            this.goto_XY(getPos(rowNow, getCurrentCol() - 1));
            simulated = true;
            break;
        ...
        case FAST_CURSOR_RIGHT:
            ...
            if (rowNow > getRows() - 1) rowNow = getRows() - rowNow;
    ...
    protected boolean simulateKeyStroke(char c) {
        ...
        if (!Character.isISOControl(c)) {
            ...
            if (screenFields.getCurrentField() != null && screenFields.withinCurrentField(lastPos) && !screenFields.isCurrentFieldBypassField()) {
                ...
                if (screenFields.isCurrentFieldFER() && !screenFields.withinCurrentField(screenFields.getCurrentFieldPos())
                    && lastPos == screenFields.getCurrentField().endPos()
                    && screenFields.getCurrentFieldPos() > screenFields.getCurrentField().endPos()) {
                ...
                case 4: // Kakana Shift
                    ...
                    break;
                case 1: // Alpha Only
                    if (Character.isLetter(c) || c == ',' || c == '-' || c == '.' || c == ' ') updateField = true;
                    break;
                case 3: // Numeric only
                ...
                case 7: // Signed numeric
                    if (Character.isDigit(c) || c == '+' || c == '-')
                        if (lastPos == screenFields.getCurrentField().endPos() && (c != '+' && c != '-'))
                ...
                if (updateField) {
                    if (screenFields.isCurrentFieldToUpper()) c = Character.toUpperCase(c);
                    ...
                    if (updatePos) {
                        ...
                        if (!screenFields.withinCurrentField(screenFields.getCurrentFieldPos())) {
                            ...
                            setCursor(screenFields.getCurrentField().getCursorRow() + 1, screenFields.getCurrentField().getCursorCol() + 1);

                    }

                    fireScreenChanged(1);

                    if (autoEnter) sendAid(AID_ENTER);
                } else {
                    if (numericError) {
    ...
    private boolean fieldExit() {
        ...
        // save off the current pos of the field for checking field exit
        // required
        ...
        // Maybe this should be changed to not update the current cursor
        // position
        // of the field.
    ...
    private void shiftLeft(int sPos) {
        ...
        do {
            ...
            count = (end - screenFields.getCurrentField().startPos()) - screenFields.getCurrentField().getKeyPos(pPos);
            ...
            if (screenFields.isCurrentFieldContinued()) {
                gotoFieldNext();
                if (screenFields.getCurrentField().isContinuedFirst()) break;
    ...
    public int getRow(int pos) {
        ...
        if (row > (lenScreen / numCols) - 1) row = (lenScreen / numCols) - 1;
     ...
     * </tr>
     * </table>
     * I am tired of typing and they should be self explanitory. Finding them in
     * the first place was the pain.
     * </p>
     * 
     * @param ec error code
     */
    ...
    private void process_XY(int pos) {
        ...
        if (pos < 0) pos = lenScreen + pos;
        if (pos > lenScreen - 1) pos = pos - lenScreen;
     ...
     * 
     * @return character array containing the text Added by Luc - LDC Note to
     *         KJP - Have to ask what the difference is between this method and
     *         the other
     */
    ...
    // throws OhioException {
    {
        ...
        return planes.GetScreenRect(buffer, bufferLength, startRow, startCol, endRow, endCol, plane);
    }
    ...
    public boolean gotoField(int f) {
        ...
        if (f > sizeFields || f <= 0) return false;
    ...
    private void gotoFieldNext() {
        ...
        if (screenFields.isCurrentFieldHighlightedEntry()) unsetFieldHighlighted(screenFields.getCurrentField());

        screenFields.gotoFieldNext();

        if (screenFields.isCurrentFieldHighlightedEntry()) setFieldHighlighted(screenFields.getCurrentField());
    }
     ...
     * @param lr
     */
    protected void createWindow(int depth, int width, int type, boolean gui, int monoAttr, int colorAttr, int ul, int upper, int ur, int left,
    ...
    /**
     * Creates a scroll bar on the screen using the parameters provided. ** we
     * only support vertical scroll bars at the time.
     * 
    ...
    protected void writeWindowTitle(int pos, int depth, int width, byte orientation, int monoAttr, int colorAttr, StringBuffer title) {
        ...
        if ((orientation & 0x20) == 0x20) pos += ((depth + 1) * numCols);
    ...
    /**
     * Roll the screen up or down. Byte 1: Bit 0 0 = Roll up 1 = Roll down Bits
     * 1-2 Reserved Bits 3-7 Number of lines that the designated area is to be
     * rolled Byte 2: Bits 0-7 Line number defining the top line of the area
     * that will participate in the roll. Byte 3: Bits 0-7 Line number defining
     * the bottom line of the area that will participate in the roll.
     * 
     * @param direction
     * @param topLine
     * @param bottomLine
    ...
    protected void setAttr(int cByte) {
        ...
        while (planes.getCharAttr(lastPos) != lastAttr && !planes.isAttributePlace(lastPos)) {

            planes.setScreenAttr(lastPos, lastAttr);
            if (guiInterface && !isInField(lastPos, false)) {
                int g = planes.getWhichGUI(lastPos);
                if (g >= FIELD_LEFT && g <= FIELD_ONE) planes.setUseGUI(lastPos, NO_GUI);
            }
            setDirty(lastPos);
    ...
    /**
     * Draw or redraw the dirty parts of the screen and display them. Rectangle
     * dirty holds the dirty area of the screen to be updated. If you want to
     * change the screen in anyway you need to set the screen attributes before
    ...
    /**
     * Change position of the screen by the increment of parameter passed. If
     * the position change is under the minimum of the first screen position
     * then the position is moved to the last row and column of the screen. If
     * the position change is over the last row and column of the screen then
    ...
    protected void changePos(int i) {
        ...
        lastPos += i;
        if (lastPos < 0) lastPos = lenScreen + lastPos;
        if (lastPos > lenScreen - 1) lastPos = lastPos - lenScreen;
    ...
    protected void setPendingInsert(boolean flag) {
        if (homePos != -1) pendingInsert = flag;
    }
    ...
    protected void setStatus(byte attr, byte value, String s) {
        ...
        case STATUS_ERROR_CODE:
            if (value == STATUS_VALUE_ON) {
                ...
                oia.setInputInhibited(ScreenOIA.INPUTINHIBITED_SYSTEM_WAIT, ScreenOIA.OIA_LEVEL_INPUT_ERROR, s);
    ...
    private synchronized void fireScreenChanged(int update) {
        ...
        fireScreenChanged(update, getRow(dirtyScreen.x), getCol(dirtyScreen.x), getRow(dirtyScreen.y), getCol(dirtyScreen.y));
    ...
    public void repaintScreen() {
        ...
        if (oia.isMessageWait()) oia.setMessageLightOn();
        setCursorOn();
    }

    // ADDED BY BARRY - changed by Kenneth to use the character plane
    // This should be replaced with the getPlane methods when they are
    // implemented
