...
public class ScreenFields {
     ...
     * Taken from Section 16.2.1.2 Enter/Rec Adv Key In the normal unlocked
     * state, when the workstation operator presses the Enter/Rec Adv key: 1.
     * The 5494 checks for the completion of mandatory-fill, self-check, and
    ...
    public boolean isCanSendAid() {
        ...
        if (currentField != null && (currentField.getAdjustment() > 0 || currentField.isSignedNumeric()) && currentModified && isInField()
    ...
    protected boolean isInField(int pos, boolean chgToField) {
        ...
        for (int x = 0; x < sizeFields; x++) {
            ...
            if (sf.withinField(pos)) {
                ...
                if (chgToField) {
                    if (!currentField.equals(sf)) currentModified = false;
                    currentField = sf;
                }
                return true;
            }
     ...
     * @param startPos The row and column where to start the search. The
     *        position is inclusive (for example, row 1, col 1 means that
     *        position 1,1 will be used as the starting location and 1,1 will be
     ...
     *        </tr>
     *        <tr>
     *        <td>OS_OHIO_DIRECTION_FORWARD</td>
     *        <td>0</td>
     *        <td>Forward (beginning towards end)</td>
     *        </tr>
     *        <tr>
     *        <td>OS_OHIO_DIRECTION_BACKWARD</td>
     *        <td>1</td>
     *        <td>Backward (end towards beginning)</td>
     *        </tr>
     *        </table>
     *        Constant Value Description ignoreCase - Indicates whether the
     *        search is case sensitive. True means that case will be ignored.
    ...
    /**
     * Searches the collection for the target position and returns the
     * ScreenField object containing that position.
     * 
     * @param targetPosition The target row and column expressed as a linear
     *        position within the presentation space.
     * @return If found, a ScreenField object containing the target position. If
     *         not found, returns a null.
     */
     ...
     * ScreenField object containing that position.
     * 
     * @param row The beginning row to start search with in the presentation
     *        space.
     * @param col The beginning column to start search with in the presentation
     *        space.
     * @return If found, a ScreenField object containing the target position. If
     *         not found, returns a null.
     */
    ...
    public void gotoFieldNext() {
        ...
        // sanity check - we where getting null pointers after a restore of
        // screen
        ...
        if (!sf.withinField(lastPos)) {
            ...
            if (sizeFields > 0) {
                ...
                // lets get the current position so we can test if we have
                // looped
            ...
            if (!cpfExists) {
                ...
                if (cp == 0) {
                    ...
                    while (!found && f < sizeFields) {
                        ...
                        sf1 = screenFields[f++];
                        if (sf1.getFieldId() == cp) found = true;
                    }
                    if (found)
                        sf = sf1;
                    else {
            ...
            if (sf.startPos() == lastPos) {
                if (!cpfExists) {
                    ...
                    while (!found && f < sizeFields) {
                        ...
                        sf1 = screenFields[f++];
                        if (sf1.getCursorProgression() == cp) found = true;
                    }
                    if (found)
                        sf = sf1;
                    else {
        ...
        if (masterMDT) {
            ...
            for (int x = 0; x < sizeFields; x++) {
                ...
                if (sf.mdt || (readType == CMD_READ_INPUT_FIELDS)) {
                    ...
                    if (readType == CMD_READ_MDT_FIELDS || readType == CMD_READ_MDT_IMMEDIATE_ALT) {
                        int len = sb.length() - 1;
                        ...
                        // )) {
                            ...
                            // if we have the dup character and dup is enabled
                            // then we
                            // stop here
                            if (sb.charAt(len) == 0x1C && sf.isDupEnabled()) break;
                    ...
                    if (len3 > 0 || (readType == CMD_READ_MDT_FIELDS || readType == CMD_READ_MDT_IMMEDIATE_ALT)) {
                        ...
                        if (sf.isSelectionField()) {
                            ...
                            for (int k = 0; k < len3; k++) {
                                c = sb.charAt(k);
                                // here we have to check for special instances
                                // of the
                                // characters in the string field. Attribute
                                // bytes
                                ...
                                if (c < ' ' || c >= '\uff20') {
                                    ...
                                    // if it is an offset attribute byte we just
                                    // pass
