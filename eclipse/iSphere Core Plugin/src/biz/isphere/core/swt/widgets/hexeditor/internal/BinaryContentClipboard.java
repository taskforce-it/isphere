 ...
 * @author Jordi
 */
public final class BinaryContentClipboard {

    public static String CLIPBOARD_FOLDER_PATH;
    public static final String CLIPBOARD_FILE_NAME = "javahexeditorClipboard.tmp"; //$NON-NLS-1$
    public static final String CLIPBOARD_FILE_NAME_PASTED = "javahexeditorPasted{0}.tmp"; //$NON-NLS-1$

    private static class FileByteArrayTransfer extends ByteArrayTransfer {
        private static final String MYTYPENAME = "myFileByteArrayTypeName"; //$NON-NLS-1$
    ...
    private static class MemoryByteArrayTransfer extends ByteArrayTransfer {
        private static final String MYTYPENAME = "myMemoryByteArrayTypeName"; //$NON-NLS-1$
        private static final int MYTYPEID = registerType(MYTYPENAME);
    ...
    static {
        File tempFolder = new File(System.getProperty("java.io.tmpdir", ".")); //$NON-NLS-1$ //$NON-NLS-2$
    ...
    public void dispose() throws IOException {
        ...
        for (File aFile : myFilesReferencesCounter.keySet()) {
            ...
            if (updateLock(lock, -count)) { // lock deleted
                if (!deleteFileALaMs(aFile)) aFile.deleteOnExit();
            }
        }
    }
    ...
    private void emptyClipboardFile() {
        if (clipboardFile.canWrite() && clipboardFile.length() > 0L) {
            try {
                RandomAccessFile file = new RandomAccessFile(clipboardFile, "rw"); //$NON-NLS-1$
                file.setLength(0L);
                file.close();
            } catch (IOException e) {
            } // ok, leave it alone
     ...
     * @return
     */
    public long getHexContents(BinaryContent content, String charset, long start, boolean insert) {
        return getContents(content, charset, start, insert);
    }

    public long getTextContents(BinaryContent content, String charset, long start, boolean insert) {
        return getContents(content, charset, start, insert);
    }

    public long getContents(BinaryContent content, String charset, long start, boolean insert) {
        ...
        // paste byte/text data
        total = tryGettingMemoryByteArray(content, charset, start, insert);
        if (total >= 0L) {
            return total;
        }
    ...
    private File getLockFromFile(File lastPaste) {
        ...
        return new File(name.substring(0, name.length() - 3) + "lock"); //$NON-NLS-1$
    }
     ...
     * @param length
     */
    public void setHexContents(BinaryContent content, long start, long length) {
        setContents(content, null, start, length);
    }

    public void setTextContents(BinaryContent content, String charset, long start, long length) {
        setContents(content, charset, start, length);
    }

    public void setContents(BinaryContent content, String charset, long start, long length) {
        ...
        try {
            if (length <= maxClipboardDataInMemory) {
                ...
                String textData;
                if (charset == null) {
                    textData = new String(byteArrayData);
                    ...
                    textData = new String(byteArrayData, charset);
    ...
    private long tryGettingFileByteArray(BinaryContent content, long start, boolean insert) {
        ...
        if (clipboardFile.equals(lastPaste)) {
            for (int i = 0; i < 9999; ++i) {
                String fileName = CLIPBOARD_FILE_NAME_PASTED.replaceFirst("{0}", String.valueOf(i)); //$NON-NLS-1$
                File clipboardDir = new File(CLIPBOARD_FOLDER_PATH);
                lastPaste = new File(clipboardDir, fileName);
                lock = new File(clipboardDir, fileName + ".lock"); //$NON-NLS-1$
                if (!lock.exists()) {
    ...
    private long tryGettingMemoryByteArray(BinaryContent content, String charset, long start, boolean insert) {
    ...
    private boolean updateLock(File lock, int references) throws IOException {
        RandomAccessFile file = new RandomAccessFile(lock, "rw"); //$NON-NLS-1$
        if (file.length() >= 4) {
