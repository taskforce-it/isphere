...
public class SpooledFileBaseFilterStringEditPane {
    ...
    private String validateOutputQueue() {
        ...
        error = validateSimple(Messages.Library, outqLibText.getText(), SPECIAL_VALUES_OUTPUT_QUEUE_LIBRARY);
        if (error != null) {
            return error;
        }
        ...
        if ("*".equals(outqText.getText()) || "*ALL".equals(outqText.getText())) {
            if (!"*".equals(outqLibText.getText())) {
                return Messages.bind(Messages.The_value_in_field_A_is_not_valid, Messages.Library);
            }
        }
