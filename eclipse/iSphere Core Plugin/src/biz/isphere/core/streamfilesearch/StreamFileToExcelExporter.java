...
public class StreamFileToExcelExporter {
    ...
    public StreamFileToExcelExporter(Shell shell, SearchOptions searchOptions, SearchResult[] searchResults) {
    ...
    public void export() {
        ...
        if (file != null) {
            ...
            try {
                ...
                exportStreamFilesWithStatements(workbook);
                exportStreamFiles(workbook);
    ...
    private void exportStreamFiles(WritableWorkbook workbook) throws Exception {
        ...
        for (int index = 0; index < searchResults.length; index++) {
            ...
            sheet.addCell(new jxl.write.Label(COLUMN_DIRECTORY, line, searchResults[index].getDirectory()));
            sheet.addCell(new jxl.write.Label(COLUMN_STREAM_FILE, line, searchResults[index].getStreamFile()));
            sheet.addCell(new jxl.write.Label(COLUMN_SOURCE_TYPE, line, searchResults[index].getType()));
        ...
        sheet.setColumnView(COLUMN_DIRECTORY, COLUMN_WIDTH_NAME);
        sheet.setColumnView(COLUMN_STREAM_FILE, COLUMN_WIDTH_NAME);
    ...
    private void exportStreamFilesWithStatements(WritableWorkbook workbook) throws Exception {

        WritableSheet sheet = workbook.createSheet(Messages.Stream_files_with_statements, 0);
        ...
        for (int index1 = 0; index1 < searchResults.length; index1++) {
            ...
            for (int index2 = 0; index2 < statements.length; index2++) {
                ...
                sheet.addCell(new jxl.write.Label(COLUMN_DIRECTORY, line, searchResults[index1].getDirectory()));
                sheet.addCell(new jxl.write.Label(COLUMN_STREAM_FILE, line, searchResults[index1].getStreamFile()));
                sheet.addCell(new jxl.write.Label(COLUMN_SOURCE_TYPE, line, searchResults[index1].getType()));
