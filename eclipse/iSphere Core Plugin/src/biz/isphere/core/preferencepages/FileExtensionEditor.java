 ...
 */
public class FileExtensionEditor extends Dialog {
    ...
    private Text decTxtFileExtension;
    ...
    public static FileExtensionEditor getEditorForNew(Shell aParent, Table aTable) {
        FileExtensionEditor tEditor = new FileExtensionEditor(aParent, SWT.NONE);
        tEditor.mode = MODE_NEW;
        tEditor.setText(Messages.FileExtensionEditor_headline_new);
        tEditor.setModel(aTable);
        return tEditor;
    }

    public static FileExtensionEditor getEditorForEdit(Shell aParent, Table aTable) {
        FileExtensionEditor tEditor = new FileExtensionEditor(aParent, SWT.NONE);
        tEditor.mode = MODE_EDIT;
        tEditor.setText(Messages.FileExtensionEditor_headline_edit);
        tEditor.setModel(aTable);
        return tEditor;
    }

    private FileExtensionEditor(Shell aParent, int aStyle) {
    ...
    private void createContents() {
        ...
        decTxtFileExtension = WidgetFactory.createText(shell);
        ...
        lblFileExtension.setText(Messages.Label_fileExtension_colon);
    ...
    private void performOK(SelectionEvent anEvent) {
        if (mode == MODE_NEW) {
            new TableItem(tblFileExtensions, SWT.NONE).setText(decTxtFileExtension.getText());
        } else {
            getEditedItem().setText(decTxtFileExtension.getText());
        }
    ...
    private void initializeValues() {
        if (mode == MODE_NEW) {
            decTxtFileExtension.setText(""); //$NON-NLS-1$
        } else {
            decTxtFileExtension.setText(editedFileExtension);
        }
    }
