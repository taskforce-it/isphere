 ...
 * variables.
 * <p/>
 * To get a variable value at first {@link #getVariable(String name)} and after
 * that {@link #findColumn(String name)} method is used.
 * <p/>
 * In expressions 'part_1.part2' names of variables are supported. Parser is
 * case sensitive.
 * <p/>
 * Visit <a
 * href="http://sourceforge.net/projects/sqljep">http://sourceforge.net/
 * projects/sqljep</a> for the newest version of SQLJEP, and complete
 * documentation.
 * 
...
public abstract class BaseJEP implements ParserVisitor {
    ...
    /** Function Table */
    private static final HashMap<String, PostfixCommandI> funTab = new HashMap<String, PostfixCommandI>();

    static {
        // add functions to Function Table
        funTab.put(Abs.ID, new Abs()); // JUnit
        funTab.put(Ceil.ID, new Ceil()); // JUnit
        funTab.put(Concat.ID, new Concat()); // JUnit
        funTab.put(Date.ID, new Date()); // JUnit
        funTab.put(Day.ID, new Day()); // JUnit
        funTab.put(Floor.ID, new Floor()); // JUnit
        funTab.put(Hour.ID, new Hour()); // JUnit
        funTab.put(Length.ID, new Length()); // JUnit
        funTab.put(Lower.ID, new Lower()); // JUnit
        funTab.put(Lpad.ID, new Lpad()); // JUnit
        funTab.put(Ltrim.ID, new Ltrim()); // JUnit
        funTab.put(Microsecond.ID, new Microsecond()); // JUnit
        funTab.put(Minute.ID, new Minute()); // JUnit
        funTab.put(Modulus.ID, new Modulus()); // JUnit
        funTab.put(Month.ID, new Month()); // JUnit
        funTab.put(Replace.ID, new Replace()); // JUnit
        funTab.put(Round.ID, new Round()); // JUnit
        funTab.put(Rpad.ID, new Rpad()); // JUnit
        funTab.put(Rtrim.ID, new Rtrim()); // JUnit
        funTab.put(Second.ID, new Second()); // JUnit
        funTab.put(Sign.ID, new Sign()); // JUnit
        funTab.put(Substring.ID1, new Substring()); // JUnit
        funTab.put(Substring.ID2, new Substring()); // JUnit
        funTab.put(Time.ID, new Time()); // JUnit
        funTab.put(Timestamp.ID, new Timestamp()); // JUnit
        funTab.put(Trim.ID, new Trim()); // JUnit
        funTab.put(Upper.ID, new Upper()); // JUnit
        funTab.put(Year.ID, new Year()); // JUnit
    }
    ...
    /**
     * Creates a new SQLJEP instance. Store String representation of the
     * expression. <br/>
     ...
     * <p/>
     * Sometimes it is necessary to change value of a SQLJEP expression by some
     * external reasons. Then following code can be applied <blockquote>
     * 
     * <pre>
     ...
     * data.
     * 
     * @return number of column in a row. Columns are numbered from 0. Negative
     *         number means column not found.
     ...
     * <p/>
     * Example of external variables are
     * <CODE>TIMESTAMP,DATE,TIME,SYSDATE</CODE> variables. Values of them should
     * be fixed before expression evaluation.
     * <p/>
     ...
     *        function
     */
    public static void addFunction(String functionName, PostfixCommandI function) {
    ...
    public static Object removeFunction(String name) {
        return funTab.remove(name.toLowerCase());
    }

    /**
     * Gets a function from the function table.
     * 
     * @param name - name of the function
     * @return function
     */
    public static PostfixCommandI getFunction(String name) {
        return funTab.get(name.toLowerCase());
    }

    /**
     * Returns true if the specified function exists, else false.
     * 
     * @param name - name of the function
     * @return true, if the function exists
     */
    public static boolean hasFunction(String name) {
        return funTab.containsKey(name.toLowerCase());
    }
     ...
     * 
     * @throws org.medfoster.sqljep.ParseException If there are
     ...
     * specified with its top node. The algorithm uses a stack
