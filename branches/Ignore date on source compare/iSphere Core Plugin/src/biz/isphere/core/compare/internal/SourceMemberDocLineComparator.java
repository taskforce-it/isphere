...
package biz.isphere.core.compare.internal;
 ...
 * it has to strip the date portion from the source line when the date must be
 * must be ignored.
 * <p>
 ...
 */
public class SourceMemberDocLineComparator implements ITokenComparator {
	 ...
	 */
	public SourceMemberDocLineComparator(IDocument document, IRegion region,
			boolean ignoreWhiteSpace, boolean ignoreDate) {
		fDocument = document;
		fIgnoreWhiteSpace = ignoreWhiteSpace;
		fIgnoreDate = ignoreDate;
	...
	public boolean rangesEqual(int thisIndex, IRangeComparator otherComparator, int otherIndex) {
		...
		if (otherComparator != null && otherComparator.getClass() == getClass()) {
			SourceMemberDocLineComparator other= (SourceMemberDocLineComparator) otherComparator;
			...
			if (fIgnoreWhiteSpace) {
				String s1= extract(thisIndex, lineOffset);
				String s2= other.extract(otherIndex, lineOffset);
	 ...
	 */
	private String extract(int line, int offset) {
		if (line < fLineCount) {
			try {
				IRegion r= fDocument.getLineInformation(fLineOffset + line);
				String l= fDocument.get(r.getOffset(), r.getLength());
				if (l.length() <= offset){
				    return l;
				}
				return l.substring(offset);
