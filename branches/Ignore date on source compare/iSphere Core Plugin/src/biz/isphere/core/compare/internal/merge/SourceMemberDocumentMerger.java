...
package biz.isphere.core.compare.internal.merge;
...
import biz.isphere.core.compare.internal.SourceMemberDocLineComparator;
 ...
 * it has been changed to use a <code>SourceMemberDocLineComparator</code>
 * instead of a <code>DocLineComparator</code>. Furthermore it passes the
 * <code>considerDate</code> option of the <CompareEditorConfiguration> to
 * the <code>SourceMemberDocLineComparator</code>.
 * <p>
 ...
 */
public class SourceMemberDocumentMerger {
...
//		}
	...
	public SourceMemberDocumentMerger(IDocumentMergerInput input) {
		this.fInput = input;
	...
	public void doDiff() throws CoreException {
		...
		boolean ignoreDate = isIgnoreDate();
		
		SourceMemberDocLineComparator sright= new SourceMemberDocLineComparator(rDoc, toRegion(rRegion), ignoreWhiteSpace, ignoreDate);
		SourceMemberDocLineComparator sleft= new SourceMemberDocLineComparator(lDoc, toRegion(lRegion), ignoreWhiteSpace, ignoreDate);
		SourceMemberDocLineComparator sancestor= null;
		if (aDoc != null) {
			sancestor= new SourceMemberDocLineComparator(aDoc, toRegion(aRegion), ignoreWhiteSpace, ignoreDate);
			/*if (isPatchHunk()) {
				if (isHunkOnLeft()) {
		...
		final SourceMemberDocLineComparator sa= sancestor, sl= sleft, sr= sright;
					...
					if (useChange(diff)) {
						...
						if (s.length() > 0 && d.length() > 0) {
							...
							if (USE_MERGING_TOKEN_DIFF)
								mergingTokenDiff(diff, aDoc, a, rDoc, d, lDoc, s, lineOffset);
							else
								simpleTokenDiff(diff, aDoc, a, rDoc, d, lDoc, s, lineOffset);
						}
					}
				}
			}
	...
	private boolean isCapped(SourceMemberDocLineComparator ancestor,
			SourceMemberDocLineComparator left, SourceMemberDocLineComparator right) {
	...
	private boolean isIgnoreDate() {
	    ...
	    if (!(config instanceof CompareEditorConfiguration)) {
	        return false;
	    }
	    return !((CompareEditorConfiguration)config).isConsiderDate();
	}
...
//				
	...
	private void simpleTokenDiff(final Diff baseDiff,
		...
		for (int i= 0; i < e.length; i++) {
			RangeDifference es= e[i];
			int kind= es.kind();
			if (kind != RangeDifference.NOCHANGE && !isIgnoredDatePortion(es)) {
	...
	private boolean isIgnoredDatePortion(RangeDifference es) {
        if (!isIgnoreDate()) {
            return false;
        }
        if (es.leftStart() != 0 || es.rightStart() != 0) {
            return false;
        }
        return true;
    }
